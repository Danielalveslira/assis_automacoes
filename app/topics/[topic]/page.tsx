...
export default async function TopicPage({ params }: Props) {
  ...
  const posts = allPosts.filter(
    (post) =>
      post.topics.filter(
        (topic) => topic.toLowerCase() === resolvedParams.topic.toLowerCase()
      ).length > 0
  );
  const topicTitle =
    resolvedParams.topic.charAt(0).toUpperCase() +
    resolvedParams.topic.slice(1).replace(/-/g, " ");
        ...
        {posts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-8">
              Estamos trabalhando para trazer conteúdo sobre este tópico em
              breve.
            </p>
            <Link
              href="/blog"
